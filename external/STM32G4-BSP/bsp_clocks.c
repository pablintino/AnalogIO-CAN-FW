...
ret_status BSP_CLK_reset_clocks(void) {
    ...
    ret_status tmp_status = BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_HSIRDY, RCC_CR_HSIRDY,
                                                          BSP_CLK_HSI_READY_TMT);
    ...
    tmp_status = BSP_UTIL_wait_flag_status_now(&RCC->CFGR, RCC_CFGR_SWS, RCC_CFGR_SWS_HSI, 5000U);
    if (tmp_status != STATUS_OK) {
        return tmp_status;
    }

    // Wait until PLL is OFF
    tmp_status = BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_PLLRDY, 0UL, BSP_CLK_PLL_READY_TMT);
    if (tmp_status != STATUS_OK) {
        return tmp_status;
    }
...
ret_status __config_clock_hsi(const bsp_clk_osc_config_t *oscc) {
    ...
    if (oscc->HSIState == BSP_CLK_CLOCK_STATE_HSI_STATE_ENABLE) {
        /* Enable HSI */
        __BSP_SET_MASKED_REG(RCC->CR, RCC_CR_HSION);
        return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_HSIRDY, RCC_CR_HSIRDY, BSP_CLK_HSI_READY_TMT);
        ...
        return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_HSIRDY, 0UL, BSP_CLK_HSI_READY_TMT);
    } else {
...
ret_status __config_clock_hse(const bsp_clk_osc_config_t *oscc) {
    ...
    if (oscc->HSEState == BSP_CLK_CLOCK_STATE_HSE_STATE_ENABLE) {
        ...
        return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_HSERDY, RCC_CR_HSERDY, BSP_CLK_HSE_READY_TMT);
        ...
        return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_HSERDY, 0, BSP_CLK_HSE_READY_TMT);
    } else {
        /* Invalid configuration option. Shouldn't be reached */
        return STATUS_ERR;
    }
...
ret_status __disable_pll() {
    ...
    return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_PLLRDY, 0UL, BSP_CLK_PLL_READY_TMT);
}
...
ret_status __pll_validate_params(const bsp_pll_config_t *pllc) {
    ...
    if (!BSP_CLK_IS_PLLP_VALID(pllc->PLLP) || !BSP_CLK_IS_PLLQ_VALID(pllc->PLLQ) ||
        !BSP_CLK_IS_PLLR_VALID(pllc->PLLR) || \
        !BSP_CLK_IS_PLLM_VALID(pllc->PLLM) || !BSP_CLK_IS_PLLN_VALID(pllc->PLLN)) {
        return STATUS_ERR;
    }
...
ret_status __config_clock_pll(const bsp_pll_config_t *pllc) {
    ...
    if (pllc->PLLState == BSP_CLK_CLOCK_STATE_PLL_STATE_ENABLE) {
        ...
        return BSP_UTIL_wait_flag_status_now(&RCC->CR, RCC_CR_PLLRDY, RCC_CR_PLLRDY, BSP_CLK_PLL_READY_TMT);
...
ret_status __config_clock_sysclk(const bsp_clk_clock_config_t *clkc) {
    ...
    return BSP_UTIL_wait_flag_status_now(&RCC->CFGR, RCC_CFGR_SWS, clkc->SystemClockSource << RCC_CFGR_SWS_Pos, 5000UL);
}
...
ret_status __config_clock_hclk(const bsp_clk_clock_config_t *clkc) {
    ...
    uint32_t temp_status = BSP_UTIL_wait_flag_status_now(&RCC->CFGR, RCC_CFGR_HPRE, clkc->AHBDivider, 100UL);
    if (temp_status != STATUS_OK) {
        return temp_status;
    }
    ...
    return BSP_UTIL_wait_flag_status_now(&RCC->CFGR, RCC_CFGR_PPRE1 | RCC_CFGR_PPRE2,
...
uint32_t __calculate_target_hclk_freq(const bsp_clk_clock_config_t *clkc) {
    ...
    return base_freq / (clkc->AHBDivider & 0x00000080U ? HPRE_DIVIDERS[
            ((clkc->AHBDivider & RCC_CFGR_HPRE) >> RCC_CFGR_HPRE_Pos) & 0x03] : 1UL);
}
...
uint8_t __calculate_flash_wait_states(uint32_t frequency) {
    ...
    uint8_t latencies =
            (PWR->CR1 & PWR_CR1_VOS) == PWR_CR1_VOS_0 ? BSP_UTL_COUNT_OF(RANGE_1_LATENCY_FREQS) : BSP_UTL_COUNT_OF(
                    RANGE_2_LATENCY_FREQS);
...
ret_status __change_flash_latency(uint8_t flash_wait_states) {
    ...
    return BSP_UTIL_wait_flag_status_now(&FLASH->ACR, FLASH_ACR_LATENCY, flash_wait_states, 10UL);
}
