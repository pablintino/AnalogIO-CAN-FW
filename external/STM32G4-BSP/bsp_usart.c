...
static uint32_t __calculate_brr(uint32_t usart_clk, uint32_t baudrate, uint16_t prescaler, enum busart_sampling_e sampling);
...
ret_status busart_config(BSP_USART_Instance *usart, const bsp_usart_config_t *config)
{
    ...
    uint32_t usart_prescaler = config->prescaler != BSP_USART_PRESCALER_1 ? USART_PRESCALERS[config->prescaler - 1] : 1;
    usart->PRESC = config->prescaler & USART_PRESC_PRESCALER;
    uint32_t usart_div = __calculate_brr(usart_frequency, config->baudrate, usart_prescaler, config->bit_sampling);
    ...
    __BSP_SET_MASKED_REG_VALUE(usart->CR1,
                               ...
                               USART_CR1_PS,
                               (config->bit_sampling & USART_CR1_OVER8) |
    ...
    __BSP_SET_MASKED_REG_VALUE(usart->CR3, USART_CR3_CTSE | USART_CR3_RTSE,
                               config->hardware_control & (USART_CR3_CTSE | USART_CR3_RTSE));
...
ret_status busart_put_char(BSP_USART_Instance *usart, uint8_t character, uint32_t timeout)
{
    ...
    return BSP_UTIL_wait_flag_status_now(&usart->ISR, USART_ISR_TC, USART_ISR_TC,
                                         timeout - (btick_get_ticks() - tickstart));
}
...
static uint32_t
__calculate_brr(uint32_t usart_clk, uint32_t baudrate, uint16_t prescaler, enum busart_sampling_e sampling)
{
